...
export default function CartPage() {
    ...
    const productSubProductPairs = useMemo(() => {
        return Array.from(new Set(cart.map((item: CartItem) => `${item.product}|${item.sub_product || ""}`)))
    }, [cart])
    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            ...
            for (const pair of productSubProductPairs as string[]) {
    ...
    const formatOptionType = (type: string) => {
        if (type === 'radio' || type === 'dropdown') return 'Choices';
        return type.charAt(0).toUpperCase() + type.slice(1);
    };

    const sortInquiryOptions = useMemo(() => (productName: string, subProduct: string | undefined, itemOptions: Record<string, any>) => {
        const key = `${productName}|${subProduct || ""}`
        const metadata = allProductOptions[key] || []
        if (metadata.length === 0) return Object.entries(itemOptions)

        return Object.entries(itemOptions).sort(([keyA], [keyB]) => {
            ...
            // 3. Quantity & Measurement Always Last
    ...
    const sortOptions = (options: any[]) => {
        ...
        return [...options].sort((a, b) => {
            const nameA = a.option_name.toLowerCase().trim().split('(')[0].trim()
            const nameB = b.option_name.toLowerCase().trim().split('(')[0].trim()
    ...
    useEffect(() => {
        if (editingItem && editingItem.product) {
            const url = `/api/products/options?productName=${encodeURIComponent(editingItem.product)}${editingItem.sub_product ? `&subProduct=${encodeURIComponent(editingItem.sub_product)}` : ""}`
            fetch(url)
                .then(r => r.json())
                .then(data => {
                    if (Array.isArray(data)) {
    ...
    }, [editingItem?.product, editingItem?.sub_product])
    ...
    const updateEditingOption = (optionName: string, value: string | string[]) => {
        ...
        setEditingItem((prev: CartItem | null) => ({
            ...prev!,
            options: {
                ...(prev!.options || {}),
                [optionName]: value
                        ...
                        <CardContent className="flex flex-col gap-6">
                            <div>
                                ...
                                <Textarea
                                    ...
                                    onChange={(e) => setDeliveryDetails((prev: any) => ({ ...prev, deliveryAddress: e.target.value }))}
                                    className="mt-1.5"
                                    rows={3}
                                />
                            </div>
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                <div>
                                    ...
                                    <Input placeholder="Enter district" value={deliveryDetails.district} onChange={(e) => setDeliveryDetails((prev: any) => ({ ...prev, district: e.target.value }))} className="mt-1.5" />
                                </div>
                                <div>
                                    <Label className="text-foreground">State <span className="text-red-500 ml-1">*</span></Label>
                                    <Input placeholder="Enter state" value={deliveryDetails.state} onChange={(e) => setDeliveryDetails((prev: any) => ({ ...prev, state: e.target.value }))} className="mt-1.5" />
                                </div>
                                <div className="sm:col-span-2 lg:col-span-1">
                                    <Label className="text-foreground">Pin Code <span className="text-red-500 ml-1">*</span></Label>
                                    <Input placeholder="Enter pin code" type="number" min="0" value={deliveryDetails.pinCode} onChange={(e) => setDeliveryDetails((prev: any) => ({ ...prev, pinCode: e.target.value }))} className="mt-1.5" />
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <Card className="w-full max-w-2xl border-border shadow-2xl relative my-auto">
                        ...
                        <CardContent className="pt-6 grid gap-6 max-h-[60vh] overflow-y-auto">
                            <div className="grid gap-4 sm:grid-cols-2">
                                ...
                                {editingItem.sub_product && (
                                    <div>
                                        <Label className="text-foreground">Product Type</Label>
                                        <Input value={editingItem.sub_product} disabled className="mt-1.5 bg-muted/50 cursor-not-allowed" />
                                    </div>
                                )}
                            </div>
                                        ...
                                        return (
                                            <div key={opt.id}>
                                                <Label className="text-foreground mb-2 flex items-center gap-2">
                                                    {opt.option_name}
                                                    {isLocked ? (
                                                        <span className="bg-primary/10 text-primary border border-primary/20 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">LOCKED</span>
                                                    ) : (
                                                ...
                                                {opt.buyer_option_type === 'dropdown' ? (
                                                    <Select
                                                        ...
                                                        onValueChange={(v) => updateEditingOption(optionKey, v)}
                                                        disabled={isLocked}
                                                    >
                                                        <SelectTrigger className={isLocked ? "bg-muted/50 grayscale-[0.5] opacity-80 cursor-not-allowed" : ""}>
                                                            <SelectValue placeholder={`Select ${opt.option_name.toLowerCase()}`} />
                                                        </SelectTrigger>
                                                        <SelectContent>
                                                    ...
                                                    <div className={`grid grid-cols-2 sm:grid-cols-3 gap-4 w-full rounded-md border border-input p-4 min-h-[40px] ${isLocked ? "bg-muted/30 opacity-80 cursor-not-allowed" : "bg-background"}`}>
                                                        {(opt.dropdown_values || []).map((val: string, idx: number) => (
                                                            <label key={idx} className={`flex items-start gap-2 text-sm leading-tight ${isLocked ? "cursor-not-allowed" : "cursor-pointer"}`}>
                                                                <input
                                                                    type="radio"
                                                                    name={`edit-${optionKey}`}
                                                                    disabled={isLocked}
                                                                    checked={(editingItem.options?.[optionKey] as string) === val}
                                                                    onChange={() => updateEditingOption(optionKey, val)}
                                                                    className={`mt-0.5 h-4 w-4 shrink-0 rounded-full border-gray-300 text-primary focus:ring-primary ${isLocked ? "opacity-50" : ""}`}
                                                                />
                                                                <span className={isLocked ? "text-muted-foreground" : ""}>{val}</span>
                                                            </label>
                                                        ))}
                                                    </div>
                                                            ...
                                                            (opt.dropdown_values || []).map((val: string, idx: number) => (
                                                                <label key={idx} className={`flex items-start gap-2 text-sm leading-tight ${isLocked ? "cursor-not-allowed" : "cursor-pointer"}`}>
                                                                    <input
                                                                        ...
                                                                        className={`mt-0.5 h-4 w-4 shrink-0 rounded border-gray-300 text-primary focus:ring-primary ${isLocked ? "opacity-50" : ""}`}
                                                                    />
                                                                    <span className={isLocked ? "text-muted-foreground" : ""}>{val}</span>
                                                                </label>
                                                            ))
                                                    ...
                                                    <Input
                                                        ...
                                                        disabled={isLocked}
                                                        className={isLocked ? "bg-muted/50 cursor-not-allowed opacity-80" : ""}
                                                    />
                                                )}
                                            </div>
                                        )
                            ...
                            <div className="border-t border-border pt-6">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setEditingItem((prev: CartItem | null) => ({ ...prev!, paymentTerms: e.target.value }))}
                                    className="mt-1.5"
                                    min="0"
                                />
                            </div>
